...
const projectsDetail = require('../model/adminModel/project');
const projectUserDetail = require('../model/adminModel/projectUserMap');
...
router.post('/addProject', checkAuth, (req, res, next) => {
    ...
    if ((req.body.projectName === "") || (req.body.projectName === null) || (req.body.projectCode === "") || (req.body.projectCode === null)) {
...
router.post('/deleteProject', checkAuth, (req, res, next) => {
    ...
    projectsDetail.deleteOne({ _id: req.body.projectId })
        .then((result => {
            ...
            projectUserDetail.deleteMany({ projectId: req.body.projectId })
                .then((result => {
                    let proUserMapDeletion = result.deletedCount;
                    console.log("source server collection deletion: " + proUserMapDeletion);
                    ...
                    if ((projectDeletion === 0) && (proUserMapDeletion === 0)) {
                        ...
                        return res.status(errorConfig.successMessageCode).json({
                            responseCode: errorConfig.successMessageCode,
                            responseDescription: "Record deleted from project-user map collection, but not deleted from project collection"
                        })
                    }
                    else if ((projectDeletion != 0) && (proUserMapDeletion === 0)) {
                        return res.status(errorConfig.successMessageCode).json({
                            responseCode: errorConfig.successMessageCode,
                            responseDescription: "Record deleted from project collection, but not deleted from project-user map user collection"
                        })
                    }
                    else if ((projectDeletion != 0) && (proUserMapDeletion != 0)) {
                        return res.status(errorConfig.successMessageCode).json({
...
router.post('/getProjectDetails', checkAuth, (req, res) => {
    ...
    projectsDetail.findOne({ _id: req.body.projectId })
        .then((result => {
            if (result === null) {
                return res.status(errorConfig.successMessageCode).json({ responseCode: errorConfig.notFoundErrorCode, responseDescription: errorConfig.notFoundErrorMsg });
            }
...
router.post('/modifyProject', checkAuth, (req, res) => {
    ...
    if ((req.body.projectName === "") || (req.body.projectName === null) || (req.body.projectCode === "") || (req.body.projectCode === null)) {
        ...
        projectsDetail.findOneAndUpdate({ _id: req.body.projectId },
            {
                projectName: req.body.projectName,
                description: req.body.description,
                projectCode: req.body.projectCode
...
router.post('/mapProjectsToUser', checkAuth, (req, res) => {
    ...
    if ((req.body.userId === "") || (req.body.userId === null) || ((req.body.projects).length === 0)) {
        return res.status(errorConfig.successMessageCode).json({ responseCode: errorConfig.badRequestCode, responseDescription: errorConfig.badRequestMsg });
    }
...
router.post('/modifyProjectsOfUser', checkAuth, (req, res) => {
    ...
    projectUserDetail.deleteMany({ userId: req.body.userId })
        .then((result => {
    ...
    if ((req.body.userId === "") || (req.body.userId === null) || ((req.body.projects).length === 0)) {
        ...
        axios({
            method: 'post', url: process.env.API_BASE_URL + '/project/mapProjectsToUser',
            data: {
                userId: req.body.userId,
