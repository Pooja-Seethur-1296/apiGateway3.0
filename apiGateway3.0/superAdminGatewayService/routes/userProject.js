...
router.post("/modifyProjectsOfUser", checkAuth, (req, res) => {
  ...
  projectUserDetail
    .deleteMany({ userId: req.body[0].userId })
    .then((result) => {
      if (result.deletedCount === 0) {
        console.log("no documents already existed");
        if (req.body.length != 0) {
          ...
          for (
            addCount = 0;
            addCount < incomingProjectsArray.length;
            addCount++
          ) {
            const projectRoleMaps = new projectUserDetail({
              _id: new mongoose.Types.ObjectId(),
            ...
            projectRoleMaps
              ...
              .then((result) => {
                ...
                if (
                  outGoingProjectsArray.length === incomingProjectsArray.length
                ) {
                  return res.status(errorConfig.successMessageCode).json({
                    userId: req.body.userId,
                    projects: outGoingProjectsArray,
