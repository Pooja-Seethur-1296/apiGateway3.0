...
module.exports = {
    ...
    feedToMongoDb: (data, callback) => {

        //map data to generate apikey
        
        let apiKey = data.map(obj => ({ ...obj, endPointToken: module.exports.generateApiKey(obj.projectCode) }));

        //insert to db

        endPointSchema.insertMany(apiKey)
            .then(result => {
                callback(result)
            })
            .catch(error => {
    ...
    checkUserIsMapped: (appProjectCode, incomingUserId, callback) => {
        projectUsers.find({userId: incomingUserId, projectCode: appProjectCode})
        .then((result) => {
            if(result.length === 0){
                callback(false)
            }else{
