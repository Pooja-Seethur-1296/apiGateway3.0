...
app.post('/reRoute', authorizeToken, (req, res) => {
    ...
    console.log(`UserId: ${req.body.userId}, API key: ${req.body.apiKey}, Request body: ${req.body.requestBody}, auth token: ${req.headers.authorization}`);
    ...
    client.hget(req.body.userId, req.body.apiKey, (error, apiUrlObj) => {
        ...
        client.hget(req.body.userId, apiAccessCount, (apiAccessErr, endPointAccessCount) => {
            try{
                if(endPointAccessCount > redisApiAccessCount){
                    return res.send({
                        responseCode: 401,
                        responseDescription: "API access is restricted, renew API subscription plan again"
                    })
                }else{
                    ...
                    client.hget(req.body.userId, reqqType, (error, reqTypeObj) => {
                        ...
                        reroute.redirect(req.body.requestBody, reqTypeObj, apiUrlObj, authorizationToken, (data, error) => {
                            try{
                                //increment redis APIAccessCount here
                                client.hincrby(req.body.userId, apiAccessCount, 1, (incData, incErr) => {
                                    if(incErr){
